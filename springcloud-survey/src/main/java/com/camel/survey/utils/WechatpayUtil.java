...
import com.camel.survey.utils.HttpRequestHandler;
...
public class WechatpayUtil
{
     ...
     */
    public static Result doTransfers(String appkey, String certPath, TransfersDto model)
    {
        ...
        try
        {
            ...
            if(result.contains("CDATA[FAIL]")){
                ...
                //2.创建输入流
                InputStream is =  StreamUtil.getStrToStream(result);
                //3.将输入流加载到build中
                Document document = saxBuilder.build(is);
                //4.获取根节点
                Element  rootElement = document.getRootElement();
                //5.获取子节点
                List<Element> children = rootElement.getChildren("err_code_des");
                return ResultUtil.error(ResultEnum.NOT_VALID_PARAM.getCode(), children.get(0).getValue());
            }
        }
        catch (Exception e)
        {
