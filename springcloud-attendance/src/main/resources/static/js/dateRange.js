...
function createSelectYear() {
    withClass("aboluo-calendar-select-year").innerHTML = '<select name="aboluo-yearSelect" id="aboluo-yearSelect" v-on:change="changeYear()" v-model="pageInfo.currentYear"></select>';
...
function getjjrszModelByYear(year) {
    ...
    AJAX.GET('/attendance/vacations/year', {vacationDay: new Date('' + year)}, function (resp) {
        if (resp.code > 0) {
            var data = resp.data;
            console.log(data)
            for (var i = 0; i < data.length; i++) {
                jjrmodelidlist.push(data[i].id);
                jjrmodeltimelist.push(dateToString(new Date(data[i].vacationDay)));
                jjrmodelztlist.push(data[i].status);
            }
        }
    });
}
...
function createMonthSelect() {
    ...
    $(selectmonth).attr('v-model', 'pageInfo.currentMonth');
    $(selectmonth).attr('v-on:change', 'changeYear()');
...
function aboluoSetrq() {
    ...
    $.ajax({
        ...
        data: {
            "vacationDay": new Date(date),
            "status": szjjr.childNodes[1].value,
            "id": ztid,
            "jjrzt": jjrzt,
