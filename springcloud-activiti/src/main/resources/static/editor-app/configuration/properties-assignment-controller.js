...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            params: {access_token: sessionStorage.getItem('access_token')},
            url: 'role/all'
        })
        ...
        $http({
            ...
            .success(function (data, status, headers, config) {
                ...
                if (data != null) {
                    var accounts = [];
                    for (var i = 0; i < obj.length; i++) {
                        accounts.push({id: obj[i].uid, code: obj[i].email, name: obj[i].username, index: i});
                    }
    ...
    $scope.save = function () {
        if ($scope.choseAssignmentFlag == "assignee") {
            // 这里获取的是name如果需要返回ID的话直接返回 $scope.formData.assignee;的值就可以了
            var choseAssignee = $scope.formData.assignee;
            var choseAssigneeName = choseAssignee;
            $scope.accounts.each(function (data, index) {
                if(choseAssignee == data.id) {
                    choseAssigneeName = data.name
                }
            });
            $scope.$emit('choseAssigneeStr', choseAssigneeName);
