...
public class WxUserController extends BaseCommonController {
    ...
    private Result save(@RequestBody WxUser wxUser) {
        ...
        if (insert) {
            jmsMessagingTemplate.convertAndSend(QUEUE_NAME, new SysUser(wxUser.getId(), wxUser.getUsername(), "123456", wxUser.getUsername(), wxUser.getEmail(), wxUser.getPhone(), wxUser.getIdNum()));
